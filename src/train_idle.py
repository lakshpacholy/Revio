"""
Model C — Idle State CNN (4-class: normal_engine_idle / low_oil / power_steering / serpentine_belt)

Architecture target: < 100 KB after TFLite conversion.
Input shape: (40, 78, 1)  — 40 MFCCs x 78 time frames x 1 channel

Class weights applied during training to correct ~2.5x imbalance
(normal_engine_idle has 2.25x more samples than fault classes).
"""
...
DATA_PATH   = ROOT / "data" / "processed" / "idle.npz"
MODEL_PATH  = ROOT / "models" / "idle_model.h5"
CURVES_PATH = ROOT / "results" / "training_curves" / "idle_curves.png"
CM_PATH     = ROOT / "results" / "confusion_matrices" / "idle_cm.png"

# ---------------------------------------------------------------------------
# Model
# ---------------------------------------------------------------------------

def build_model(input_shape: tuple = (40, 78, 1), n_classes: int = 4):
    """Lightweight CNN with slightly wider head for 4-class problem."""
    ...
    model = models.Model(inp, out, name="idle_cnn")
    model.compile(
...
def load_splits(data_path: str | Path = DATA_PATH):
    """Load pre-processed idle splits and add channel dimension."""
...
def compute_class_weights(y_train: np.ndarray) -> dict:
    """Balanced class weights to correct for normal_engine_idle overrepresentation."""
...
def plot_training_curves(history, save_path: Path = CURVES_PATH):
    ...
    fig.suptitle("Model C — Idle CNN Training Curves", fontweight="bold")
    plt.tight_layout()
...
def plot_confusion_matrix(y_true, y_pred, label_names, save_path: Path = CM_PATH):
    ...
    short = [l.replace("normal_engine_idle", "normal")
              .replace("power_steering", "pwr_steer")
              .replace("serpentine_belt", "serp_belt")
    ...
    fig.suptitle("Model C — Idle CNN Confusion Matrix (Test Set)", fontweight="bold")
    plt.tight_layout()
    plt.savefig(save_path, bbox_inches="tight")
    plt.close()
    print(f"Saved -> {save_path}")
...
def train(
    ...
    # -- Training (with class weights for imbalance correction) --
    history = model.fit(
        X_train, y_train,
