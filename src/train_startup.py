"""
Model B — Start-Up State CNN (3-class: normal_engine_startup / bad_ignition / dead_battery)
...
DATA_PATH   = ROOT / "data" / "processed" / "startup.npz"
MODEL_PATH  = ROOT / "models" / "startup_model.h5"
CURVES_PATH = ROOT / "results" / "training_curves" / "startup_curves.png"
CM_PATH     = ROOT / "results" / "confusion_matrices" / "startup_cm.png"

# ---------------------------------------------------------------------------
# Model
# ---------------------------------------------------------------------------

def build_model(input_shape: tuple = (40, 78, 1), n_classes: int = 3):
    """Lightweight CNN — same backbone as braking_cnn, 3-class softmax head."""
    ...
    model = models.Model(inp, out, name="startup_cnn")
    model.compile(
...
def load_splits(data_path: str | Path = DATA_PATH):
    """Load pre-processed startup splits and add channel dimension."""
...
def plot_training_curves(history, save_path: Path = CURVES_PATH):
    ...
    fig.suptitle("Model B — Start-Up CNN Training Curves", fontweight="bold")
    plt.tight_layout()
...
def plot_confusion_matrix(y_true, y_pred, label_names, save_path: Path = CM_PATH):
    ...
    short = [l.replace("normal_engine_startup", "normal")
              .replace("bad_ignition", "bad_ignition")
              .replace("dead_battery", "dead_battery")
    ...
    fig.suptitle("Model B — Start-Up CNN Confusion Matrix (Test Set)", fontweight="bold")
    plt.tight_layout()
    plt.savefig(save_path, bbox_inches="tight")
    plt.close()
    print(f"Saved -> {save_path}")
...
def train(
    ...
    Small-data regime: a single seed's initial weights can land the model in
    a basin it never escapes. Model selection is done purely on val_loss
    across seeds — test set is touched only once, after the winning seed.
    """
    import tensorflow as tf
